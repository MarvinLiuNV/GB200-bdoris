...
supported_kernels_ubuntu = ["4.15.0", "5.3.0", "5.4.0", "5.15.0","6.8.0"]
...
# FIXME: Remove openssh from the list once https://bugzilla.mindrot.org/show_bug.cgi?id=3756 is fixed
...
class Updater(object):
    ...
    def __init__(
        ...
        self._skip_rocev2_config = skip_rocev2_config
        ...
        if self._linux_distribution_name == "ubuntu":
            cmds_to_execute = [
                [
                    "rm",
                    "-f",
                    "/etc/apt/sources.list.d/exascaler-client.list",
                ]
            ]
            cmd_header = ["DEBIAN_FRONTEND=noninteractive"]
            ...
            yum_base_cmd = ["yum", "-y"]
            cmds_to_execute = [["rm", "-f", "/etc/yum.repos.d/exascaler-client.repo"]]
            ...
            cmds_to_execute.append(
                [
                    ...
                    (
                        ...
                        if self._mofed_installed
                        else "" if not self._disable_o2ib else "--with-o2ib=no"
                    ),
                ]
            )
    ...
    def _setup_install_lustre_client_dkms(self):
        if self._linux_distribution_name == "ubuntu":
            ...
            centos_version = "el{}".format(self._linux_distribution[1].replace(".", ""))
            repo_content = """
[exa-client]
...
gpgcheck=0
    ...
    def get_lustre_debs_installed(self):
        ...
        packages = [
            p
            for p in packages
            if "lustre-client-repo" not in p and "lustrefs_exporter" not in p
        ]
        return packages
    ...
    def _ge_mofed_versions(self, version1, version2):
        # Replace dash with a dot to handle versions like 5.8-1.1.2.1
        version1 = version1.replace("-", ".")
        version2 = version2.replace("-", ".")

        # Split each version string into components and convert to integers
        ...
        # If one version string has more components than the other
        # and all previous components are equal, the longer one is considered greater
        return len(v1_components) > len(v2_components)

    def _tune_nics(self, nics):
        # Convert aliases to actual nic
        try:
            nics_without_aliases = list(
                map(lambda nic: re.sub(r":[0-9]+", "", nic), nics)
            )
        ...
        if rc != 0:
            self._print_info(
                "lshw isn't available, nics tuning skipped", LOG_INFO_LEVEL
            )
            return False
        ...
        rc, out = self._run_cmd(cmd)
        if rc != 0:
            self._print_info("lshw failed, nics tuning skipped", LOG_INFO_LEVEL)
            return False
        try:
            ...
            self._print_info(
                "Can't read lshw xml output, nics tuning skipped", LOG_INFO_LEVEL
            )
            return False
        ...
        if len(nics_with_ethernet) <= 0:
            ...
            # RoCEv2 tunings
            if not self._skip_rocev2_config:
                if yes_no_query(
                    f"\nConfigure RoCEv2 for each o2ib with mlx Ethernet nics \n- RoCE mode v2\n- Trust DSCP\n- PFC on TC3\n- Default ToS 96\n- DCQCN RP/NP enabled for TC3\n- Disable global pause\n",
                    "yes",
                    self._yes,
                ):
                    for item in self._lnets:
                        match = re.match(r"o2ib.*?\((.*?)\)", item)
                        if match:
                            # Extract interfaces and split by comma with any number of spaces
                            interfaces = re.split(r",\s*", match.group(1))
                            # Check if all interfaces are in mlx_nics
                            # If that's the case we can ask if the user wants to tune for RoCEv2
                            if all(nic in mlx_nics for nic in interfaces):
                                self._print_info(
                                    f"\nConfiguring RoCEv2 for {item}...\n",
                                    LOG_INFO_LEVEL,
                                    log_to_console=True,
                                )
                                try:
                                    cmd = ["ibdev2netdev", "-v"]
                                    rc_ibdev2netdev, ibdev2netdev = self._run_cmd(cmd)
                                    if rc_ibdev2netdev != 0:
                                        raise Exception("ibdev2netdev failed")
                                    for nic in interfaces:
                                        # Find the device for the interface name
                                        nic_line = re.search(
                                            r".*" + nic + r".*", ibdev2netdev
                                        )
                                        if nic_line == None:
                                            raise Exception(
                                                "Can't find interface {}".format(nic)
                                            )
                                        device = re.search(
                                            r"mlx[0-9]+_[0-9]+", nic_line.group(0)
                                        )
                                        if device == None:
                                            raise Exception(
                                                "Can't find device in mst status"
                                            )
                                        device_str = str(device.group(0))
                                        # Set RoCEv2 mode on the interface
                                        cmd = [
                                            "cma_roce_mode",
                                            "-d",
                                            device_str,
                                        ...
                                        rc, dummy = self._run_cmd(cmd)
                                        if rc != 0:
                                            raise Exception(
                                                f"cma_roce_mode failed for {device}"
                                            )

                                        # Set trust to DSCP field
                                        cmds = [
                                            ...
                                            [
                                                "bash",
                                                "-c",
                                                f"'if [ -f /sys/class/net/{nic}/ecn/roce_rp/enable/3 ]  ; then echo 1 > /sys/class/net/{nic}/ecn/roce_rp/enable/3 ; else exit 1 ; fi'",
                                            ],
                                            [
                                                "bash",
                                                "-c",
                                                f"'if [ -f /sys/class/net/{nic}/ecn/roce_rp/enable/3 ]  ; then echo 1 > /sys/class/net/{nic}/ecn/roce_np/enable/3 ; else exit 1 ; fi'",
                                            ],
                                            [
                                                "ethtool",
                                                "-A",
                                    ...
                                    self._print_info(f"Error {err}")
                                    self._print_info(
                                        f"Can't tune {item} for RoCEv2. Skipping",
                                        LOG_WARNING_LEVEL,
                                    )

                else:
                    # Update skip_rocev2_config to reflect user choice
                    self._skip_rocev2_config = True
        ...
        # Relax ordering tuning (mlx only)
        if len(mlx_nics) <= 0:
            self._print_info(
        ...
        # Weeks of work can be spent on RO if misconfigured
        # Emphasis the warning message
        ro_performance_degradation_msg = (
            "\n**** !!! Severe performance degradation may occur !!! ****"
        )
        relax_ordering_set_on_at_least_one_if = False

        # Check if Relax Ordering would be beneficial or not:
        # For interfaces that have RlxdOrd+ at PCI level,
        # if MOFED is >= 5.1-0.6.6.0, then we don't *absolutly* need to force RO
        # MLX5 buffers will benefit from RO by default.
        # However, if MOFED is < or if we can't check the value, we need RO
        # If RlxdOrd- then we can't even set RO so there is no good value
        # in that case we print a warning per nic that is misconfigured.
        # https://docs.nvidia.com/networking/display/MLNXOFEDv583070LTS/Release+Notes+Change+Log+History
        forced_relaxed_ordering_is_necessary = True
        try:
            # Get the mofed version
            cmd = ["ofed_info", "-n"]
            rc, mofed_version = self._run_cmd(cmd)

            if not self._ge_mofed_versions(mofed_version, "5.1-0.6.6.0"):
                self._print_info(
                    "\nMOFED seems too old ({} < 5.1-0.6.6.0).\nForce relaxed ordering is necessary.".format(
                        mofed_version.strip()
                    ),
                    LOG_INFO_LEVEL,
                    log_to_console=True,
                ...
                self._print_info(
                    "\nYour MOFED version seems recent enough ({} >= 5.1-0.6.6.0).\nForced relaxed ordering might not be necessary".format(
                        mofed_version.strip()
                    ),
                    LOG_INFO_LEVEL,
                    log_to_console=True,
                )
                forced_relaxed_ordering_is_necessary = False
        except Exception as err:
            self._print_info("Can't check mofed version.", LOG_WARNING_LEVEL)
            # If the check didn't work we consider we need to force RO
            self._print_info(err, LOG_INFO_LEVEL)

        # Print a hint based on the MOFED version we found.
        if forced_relaxed_ordering_is_necessary:
            self._print_info(
                "It appears your system might benefit from forced relaxed ordering (up-to 4x speed-up)",
                LOG_WARNING_LEVEL,
            )

        # Ask the user if he wants to force relax ordering.
        # Skip if we used -j or answered no to Apply Relax ordering
        apply_ro_tuning = False
        if not self._skip_ro_tuning:
            if yes_no_query("\nForce Relax Ordering (mlx)", "yes", self._yes):
                apply_ro_tuning = True
            else:
                # Update skip_ro_tuning to reflect user choice
                self._skip_ro_tuning = True

        # Search the PCIe/IF/dev name mapping
        # Redundancy here as all the checks and RO tunings below rely on that mapping
        cmd = ["mst", "status", "-v"]
        rc1_mst_status, mst_status = self._run_cmd(cmd)
        if rc1_mst_status != 0:
            self._print_info("mst status failed", LOG_INFO_LEVEL)
        cmd = ["ibdev2netdev", "-v"]
        rc2_ibdev2netdev, ibdev2netdev = self._run_cmd(cmd)
        if rc2_ibdev2netdev != 0:
            self._print_info("ibdev2netdev failed", LOG_INFO_LEVEL)
        if rc1_mst_status != 0 and rc2_ibdev2netdev != 0:
            self._print_info(
                "Can't check/tune relax ordering (mst status/ibdev2netdev failed). {}".format(
                    ro_performance_degradation_msg
                ),
                LOG_WARNING_LEVEL,
            )
        ...
        # For each mlx nic, check & force relax ordering (if asked)
        # Does the check even if the user doesn't want to force RO.
        # This is because we want to catch misconfigured nics at PCI level
        # The loop can take a little while to complete
        self._print_info(
            "Checking the PCI configuration of each nic... (this can take a minute)",
            LOG_INFO_LEVEL,
            log_to_console=True,
        )
        for nic in mlx_nics:
            try:
                # 0. Search nic line
                # - try with the result of mst status
                # - try with the result of ibdev2netdev
                nic_line = None
                if rc1_mst_status == 0:
                    nic_line = re.search(r".*" + nic + r".*", mst_status)
                if nic_line == None and rc2_ibdev2netdev == 0:
                    nic_line = re.search(r".*" + nic + r".*", ibdev2netdev)
                if nic_line == None:
                    raise Exception("Can't find interface {}".format(nic))

                try:
                    # 1. Check if RlxdOrd is on at PCI level for the nic

                    # Search PCIe number ([0-9a-fA-F]{2}:[0-9a-fA-F]{2}\.[0-9])
                    pcie_number = re.search(
                        r"[0-9a-fA-F]{2}:[0-9a-fA-F]{2}\.[0-9]", nic_line.group(0)
                    )
                    if pcie_number == None:
                        raise Exception("Can't find PCI number of {}".format(nic))

                    # Look up PCIe configuration and print a warning if disabled
                    cmd = ["lspci", "-vv", "-s", pcie_number.group(0)]
                    rc, lspci_status = self._run_cmd(cmd)
                    if rc != 0:
                        raise Exception(
                            "Can't parse current lspci configuration for {}".format(nic)
                        )

                    # Search for RlxdOrd+/-
                    current_pcie_ordering = re.search(r"RlxdOrd(\+|-)", lspci_status)
                    if current_pcie_ordering == None:
                        raise Exception(
                            "Can't find RlxdOrd+/- at PCIe level for {}".format(nic)
                        )

                    # If RlxOrd is -, print a warning
                    relax_ordering_disabled_pcie = re.search(
                        r"-", current_pcie_ordering.group(0)
                    )

                    if relax_ordering_disabled_pcie != None:
                        self._print_info(
                            "Relax ordering is disabled at PCIe level for {}. {}".format(
                                nic, ro_performance_degradation_msg
                            ),
                            LOG_WARNING_LEVEL,
                        )
                        no_error = False
                        # Skip the RO tuning for that nic, disabled at PCIe level anyway
                        continue
                    else:
                        # RlxOrd is +
                        self._print_info(
                            "Relax Ordering seems enabled at PCIe level for {}".format(
                                nic
                            ),
                            LOG_INFO_LEVEL,
                        )
                    ...
                    self._print_info(err, LOG_INFO_LEVEL)
                    self._print_info(
                        "Unable to verify relax ordering status of {} at PCI level. {} if Relaxed Ordering is disabled at PCI level.".format(
                            nic, ro_performance_degradation_msg
                        ),
                        LOG_WARNING_LEVEL,
                    )
                    no_error = False
                try:
                    # 2. Force RlxdOrd if asked
                    # Search device
                    device = re.search(r"mlx[0-9]+_[0-9]+", nic_line.group(0))
                    if device == None:
                    ...
                    if relax_ordering == None:
                        ...
                        elif forced_relaxed_ordering_is_necessary:
                            # We known:
                            # - RO is not forced on the nic
                            # - MOFED is too old
                            # - user wanted to skip it
                            # => performance degradation to expect (/2 /3)
                            self._print_info(
                                "Relax ordering is not forced on {} and was skipped by user request.{}".format(
                                    nic, ro_performance_degradation_msg
                                ),
                                LOG_WARNING_LEVEL,
                            )
                        else:
                            # MOFED is recent enough. Even if RO isn't set we don't print a warning.
                            self._print_info(
                                "Relax ordering is not forced on {} and was skipped by user request but it might not be necessary".format(
                                    nic
                                ),
                                LOG_INFO_LEVEL,
                            )
                        ...
                        self._print_info(
                            "Relax ordering already set on {}".format(nic),
                            LOG_INFO_LEVEL,
                        )
                    ...
                    if forced_relaxed_ordering_is_necessary:
                        self._print_info(
                            "Can't force relax ordering on {}. {}".format(
                                nic, ro_performance_degradation_msg
                            ),
                            LOG_WARNING_LEVEL,
                        )
                        ...
                        self._print_info(
                            "Can't force relax ordering on {}.".format(nic),
                            LOG_WARNING_LEVEL,
                        )
                    no_error = False
            except Exception as err:
                self._print_info(err, LOG_INFO_LEVEL)
                self._print_info(
                    "Unable to verify and adjust relax ordering for {}. {}".format(
                        nic, ro_performance_degradation_msg
                    ),
                    LOG_WARNING_LEVEL,
                )
        ...
        if relax_ordering_set_on_at_least_one_if:
            self._print_info(
                "Relax ordering has been modified, please reboot to apply",
                LOG_INFO_LEVEL,
                log_to_console=True,
            )
        return no_error
...
"""
    ...
    def _configure_persistent(self):
        ...
        if self._skip_rocev2_config:
            tunings.append("--skip-rocev2-config")
...
WantedBy=default.target
    ...
    def _configure_lnet(self):
        ...
        # Tune ethernet nics
        if not self._tune_nics(self._nics):
            self._print_info(
                "Some nics tunings have failed, please look up EXA/A3I documentation",
                LOG_WARNING_LEVEL,
            )
    ...
    def _query_mount_cmds_emf_api(self, emf, fsnames):
        ...
        for fs in fsnames:
            ...
            try:
                ...
                self._print_info(
                    "\nAn error occured while parsing EMF API output",
                    LOG_ERROR_LEVEL,
                )
            mnts.append([fs, "".join(output["data"]["client"]["mountCommand"])])
        return mnts
    ...
    def _query_filesystems_emf_api(self, emf):
        ...
        fsnames = []
        for fs in output["data"]["filesystem"]["list"]:
            ...
            if not self._dkms:
                ...
                if self._linux_distribution_name in RHEL_DISTROS:
                    centos_version = "el{}".format(
                        self._linux_distribution[1].replace(".", "")
                    )
...
def parse_options():
    ...
    p.add_argument(
        "-o",
        "--skip-rocev2-config",
        dest="skip_rocev2_config",
        action="store_true",
        help="Skip RoCEv2 configuration",
        default=False,
    )
    p.add_argument(
        "--dkms",
...
def run_deploy_procedure():
    ...
    updater = Updater(
        ...
        skip_rocev2_config=options.skip_rocev2_config,
        yes=options.yes,
